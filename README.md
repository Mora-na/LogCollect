...
> 环境：Apple Silicon / Azul Zulu JDK 8/11/17/21 / `-Xms2g -Xmx2g -XX:+UseG1GC` / 异步文件输出

**关键吞吐（logs/s）**

| 场景 | JDK8 | JDK11 | JDK17 | JDK21 |
|------|-----:|------:|------:|------:|
| `baseline-1t-nop` | 8,261,163 | 6,842,744 | 5,849,029 | 3,867,026 |
| `isolated-8t-clean` | 5,393,268 | 5,577,944 | 6,204,150 | 6,070,060 |
| `isolated-8t-sensitive` | 815,839 | 625,756 | 659,003 | 590,198 |
| `e2e-1t-clean` | 960,948 | 1,107,837 | 1,060,993 | 1,098,499 |
| `e2e-8t-clean` | 1,169,604 | 1,308,709 | 1,403,311 | 1,364,932 |
| `e2e-8t-sensitive` | 415,871 | 541,861 | 632,635 | 597,348 |
| `degrade` | 3,113 | 3,092 | 3,116 | 3,107 |

**关键延迟（ns/log）**

| 场景 | JDK8 | JDK11 | JDK17 | JDK21 |
|------|-----:|------:|------:|------:|
| `e2e-8t-clean` | 855 | 764 | 713 | 733 |
| `e2e-8t-sensitive` | 2,405 | 1,845 | 1,581 | 1,674 |

**关键结论**

- `e2e-8t-clean` 以 JDK17 最优（1.4M logs/s），JDK21 次之（97%），JDK11（93%），JDK8（83%）。
- 敏感消息（脱敏+净化）吞吐约为干净消息的 35%~45%，安全链路成本符合预期。
- `isolated-8t-clean` 各 JDK 均达 5.4M~6.2M logs/s，框架内部流水线高并发下维持高吞吐。
- `degrade` 各 JDK 稳定在 ~3.1k logs/s，可靠性路径行为一致。
...
# 多 JDK 基线更新
./logcollect-benchmark/scripts/run-stress-multi-jdk.sh
