...
│    FastPathDetector 预扫描命中时直接快速透传                  │
│    content / throwable 与 MDC 用户字段按需净化脱敏             │
│    thread / logger / level 作为可信来源直接透传               │
│    ↓                                                         │
│ 6. 模式分发                                                   │
│    ├─ SINGLE:    构建 LogEntry → SingleModeBuffer             │
│    └─ AGGREGATE: processRawRecord → offerRaw → AggregateModeBuffer│
│    ↓                                                         │
...
```
                         ...
                         │  ② sanitize(content)   → 按需净化   │
                         │  ③ sanitizeThrowable() → 按需净化   │
                         │  ④ mask(content/throwable) → 按需脱敏│
                         │  ⑤ 输出 ProcessedLogRecord          │
                         └───────────────┬───────────────────┘
                                         │ 安全字段
                    ┌────────────────────┴──────────────────────┐
                    │                                           │
...
| 适用场景 | 逐条入库（明细表）、失败批次重试 | 大批量日志聚合到单条记录（推荐） |
...
- **flush 动作**：drain 全部 entry → 循环调用 `handler.appendLog(context, entry)`（失败走 `ResilientFlusher` 批级重试；异步 flush 使用非阻塞退避调度，最终收尾保持同步重试语义）
...
- **flush 动作**：drain 全部片段 → `StringBuilder` 拼接 → 构建 `AggregatedLog` → 一次调用 `handler.flushAggregatedLog()`（失败走 `ResilientFlusher` 批级重试与本地兜底）
...
**全局内存管控器**：控制所有活跃 `@LogCollect` 方法的缓冲区总内存占用，防止多个并发收集任务同时积压导致 OOM。  
配置项：`logcollect.global.buffer.total-max-bytes`（默认 `100MB`）+ `logcollect.global.buffer.counter-mode`（默认 `EXACT_CAS`，可选 `STRIPED_LONG_ADDER`）。
...
| 条数统计 | `LongAdder.increment()/sum()` | ✅ 分段累加 |
| 字节统计 | `LongAdder.add()/sum()` | ✅ 分段累加 |
...
| 全局内存 | `AtomicLong` CAS（默认）/ `LongAdder` 分段计数（可选） | ✅ CAS |
| 异步 flush 调度 | `ThreadPoolExecutor + LinkedBlockingQueue + CallerRunsPolicy` | ❌ Lock-based |

> 日志收集热路径无锁（入队、计数、阈值判断均基于 CAS）。  
> 异步 flush 调度线程池使用 `LinkedBlockingQueue`，属于锁实现；线程池饱和时 `CallerRunsPolicy` 触发自然反压，避免静默丢任务。
...
public final class LogEntry {
    ...
    public long estimateBytes() { ... }      // 构造期缓存估算值，调用 O(1)
...
> `handlerTimeoutMs` 基于“当前线程执行 + 定时中断”实现超时保护（`Thread.interrupt()`）。  
...
在默认实现下，`FastPathDetector` 会先做位掩码预扫描：干净文本直接走快速路径；命中可疑字符时再进入完整净化/脱敏。
`thread`/`logger`/`level` 视为框架可信来源直接透传，MDC 仅对用户自定义 key 的 value 执行净化与脱敏。
...
第三层：兜底存储
  FILE           → 安全文件（UUID 名 + 权限 600 + 空间上限 + TTL 90天 + 可选加密）
                 → 默认异步单线程写入（与 flush 线程池隔离）；`blockWhenDegradeFail=true` 时保持同步写入
...
# EXACT_CAS（精确阈值）/ STRIPED_LONG_ADDER（高并发吞吐优先，允许瞬时轻微超配额）
...
为控制标签基数，`method` 标签会收敛为 `ClassName#method`，部分低频 `reason` 会归并为 `filtered` / `backpressure`。
方法级热点计数器会在上下文初始化阶段预注册并缓存引用，避免热路径重复构建标签与查找。
...
| `logcollect.security.fastpath.hits.total` | `method` | 安全快速路径命中次数 |
...
| `logcollect-log4j2-adapter` | Appender 对 `MutableLogEvent` 改为“字段级防御复制”（保留必要复制，移除冗余 `copyString`）；新增 `Log4j2ConsolePatternDetector` |
...
| 维度 | 关键变更 |
|------|---------|
| 安全 | 新增 `StringLengthGuard`；`SecurityPipeline` 统一单入口；`FastPathDetector` 位掩码快速路径；`thread/logger/level` 可信字段透传；MDC 框架内置 key 透传、用户 key/value 按需净化脱敏；堆栈注入标记 `[ex-msg]` |
...
| 并发与性能 | 缓冲区 `ConcurrentLinkedQueue + LongAdder`；flush 后二次阈值检查；`@LogCollect` Handler 解析按 `Method` 缓存并在配置刷新后失效；AGGREGATE 按 pattern 版本切批；`AggregateModeBuffer.offerRaw` 默认走 `formatRaw`（自定义 `formatLogLine` 自动回退）；Appender 无参数消息优先模板直取，避免不必要 `getFormattedMessage()`；`AsyncFlushExecutor` 使用 `CallerRunsPolicy` 并支持 `logcollect.global.flush.*` 调参 |
| 性能与解耦 | Appender/Buffer/内存管控以及 ConfigResolver 配置刷新回调的 Metrics 调用统一改为 `logcollect-api` 接口直调（`LogCollectMetrics` / `TransactionExecutor`），移除 `invokeReflective` 热路径；`SecurityPipeline` 会话级复用；`processRawRecord` + `offerRaw` 减少 AGGREGATE 中间对象；Metrics 方法级计数器预注册并缓存引用；MDC 惰性拷贝；`LogEntry.estimateBytes()` 构造期缓存；`%d` 时间格式秒级缓存 |
...
- Part 0：`baseline-*`（Logger -> NOP 基线）
  - 仅测 `Event` 构造 + SLF4J 参数化 + NOP appender，用于剥离框架净开销。
...
- 框架 Appender 由自动装配挂到 `ROOT`，`com.logcollect.benchmark.stress.scenario` 必须 `additivity=true`，确保 e2e 场景日志传播到 `ROOT` 并经过 LogCollect Appender。
- `com.logcollect.benchmark.baseline` 使用 `additivity=false + NOP`，显式绕开 LogCollect 流水线作为基线。
...
- Stress smoke 默认 `100_000` 次/线程，并在执行前进行 `3` 秒预热，减少 JIT 冷启动噪声。
...
当前 `StressCIGateTest` 使用三道门禁：
- `isolated-1t-clean > 70,000/s`
- `isolated-8t-clean > 280,000/s`
- `isolated-8t / isolated-1t > 3.0`
`e2e-8t-clean` 在修复 logger 传播配置后改为观察项，不再使用旧的失真绝对阈值。
...
- `StressTestRunner.SMOKE_ITERATIONS`：smoke 每线程日志量（默认 `100_000`）
- `StressTestRunner.WARMUP_SECONDS`：压测前预热时长（默认 `3` 秒）
...
| 需要保留逐条明细并对失败批次重试 | **SINGLE** | 逐条落库 + 批级重试，兼顾可追溯与吞吐 |
...
| `SingleModeBuffer` / `AggregateModeBuffer` | `ConcurrentLinkedQueue` + `LongAdder` + CAS flush 互斥 |
