...
public class LogCollectLogbackAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    protected void append(ILoggingEvent event) {
        ...
        try {
            ...
            String mdcTraceId = mdc == null ? null : mdc.get(MDC_KEY);
            ...
            addError("LogCollect appender error", e);
            LogCollectInternalLogger.error("Logback appender error", e);
        } catch (Error error) {
            rethrowDegradeIfNecessary(context, error);
            addError("LogCollect appender fatal error", error);
            LogCollectInternalLogger.error("Logback appender fatal error", error);
            throw error;
        }
    }
        ...
        if (queue instanceof PipelineRingBuffer) {
            ...
            if (isWarnOrAbove(level)) {
                RawLogRecord overflow = new RawLogRecord(
                        ...
                        event.getThreadName(),
                        event.getTimeStamp(),
                        mdc == null ? Collections.emptyMap() : mdc,
                        context);
